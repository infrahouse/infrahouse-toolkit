...
def instance() -> MagicMock:
    ...
    mock = MagicMock()
    mock.db_instance_id = "ux-labs"
    mock.engine = "mysql"
    mock.engine_version = "8.0.45"
    mock.status = "available"
    mock.free_storage_bytes = 180 * GIB
    mock.slow_log_files = []
    mock.parameter_group.name = "ux-labs-pg"
    mock.parameter_group.is_default = False
    mock.parameter_group.attached_instance_ids = ["ux-labs"]
    mock.parameter_group.value_of.return_value = "0"
    mock.parameter_group.snapshot.return_value = [
...
def test_preflight_rejects_shared_parameter_group(instance: MagicMock, capture: SlowLogCapture) -> None:
    ...
    instance.parameter_group.attached_instance_ids = ["ux-labs", "ux-labs-replica"]
    with pytest.raises(SlowLogCaptureError) as exc_info:
        capture.preflight()
...
def test_start_writes_state_file_before_applying(capture: SlowLogCapture, instance: MagicMock) -> None:
    ...
    assert state["instance_id"] == "ux-labs"
    assert state["parameter_group"] == "ux-labs-pg"
...
def test_restore_from_state_file(tmp_path) -> None:
    ...
    with patch("infrahouse_toolkit.aws.rds.slow_log_capture.RDSParameterGroup") as mock_group, patch(
        ...
        assert SlowLogCapture.restore_from_state_file(str(state_file), MagicMock()) == "ux-labs"
...
def test_restore_from_malformed_state_file(tmp_path) -> None:
    ...
    state_file = tmp_path / "state.json"
    state_file.write_text('{"instance_id": "ux-labs"}')
    with pytest.raises(SlowLogCaptureError) as exc_info:
