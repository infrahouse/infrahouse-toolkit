...
def rds_client() -> MagicMock:
    ...
    client.describe_db_parameter_groups.return_value = {
        "DBParameterGroups": [{"DBParameterGroupName": "ux-labs", "DBParameterGroupFamily": "mysql8.0"}]
    }
            ...
            paginator.paginate.return_value = [
                {
                    "DBInstances": [
                        {
                            "DBInstanceIdentifier": "ux-labs-1",
                            "DBParameterGroups": [{"DBParameterGroupName": "ux-labs"}],
                        },
                        {
                            "DBInstanceIdentifier": "other",
                            "DBParameterGroups": [{"DBParameterGroupName": "somewhere-else"}],
                        },
                    ]
                }
            ]
...
def group(rds_client: MagicMock) -> RDSParameterGroup:
    ...
    return RDSParameterGroup("ux-labs", session=session)
...
def test_attached_instance_ids(group: RDSParameterGroup) -> None:
    ...
    assert group.attached_instance_ids == ["ux-labs-1"]
...
def test_is_default() -> None:
    ...
    assert RDSParameterGroup("ux-labs", session=MagicMock()).is_default is False
...
def test_apply_sets_parameters_immediately(group: RDSParameterGroup, rds_client: MagicMock) -> None:
    ...
    assert kwargs["DBParameterGroupName"] == "ux-labs"
